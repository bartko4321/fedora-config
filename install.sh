...
# ==========================================================
# KOMPLEKSOWY SKRYPT KONFIGURACYJNY SYSTEMU FEDORA 44
# ==========================================================
...
# --- Brave ---
log_info "Konfiguracja repozytorium i instalacja Brave Browser..."
...
# --- Firmware WiFi ---
log_info "Instalacja firmware WiFi (Broadcom/Intel)..."
wait_for_rpm_lock
for pkg in broadcom-wl; do
    if sudo dnf5 info "$pkg" &>/dev/null 2>&1; then
        sudo dnf5 install -y "$pkg" \
            && log_ok "Zainstalowano: $pkg" \
            || log_warn "Instalacja $pkg nie powiodła się (ignoruję)"
    else
        log_warn "Pakiet $pkg niedostępny w repozytoriach — pomijam"
    fi
done
...
# libvirt — Fedora używa virtqemud (modular daemon)
LIBVIRT_SVC=""
